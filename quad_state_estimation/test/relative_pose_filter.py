...
from quaternion_helper import quaternion_exp, skew_symm, quaternion_log, quaternion_norm, exponential_map
...
class RelativePoseFilter(object):
    ...
    def filter_update(self):
        ...
        if self.multirate_EKF and perform_apriltag_correction:
            ...
            x_hat, P_hat = self.correction_step(x_check, mu_check, P_check, 0, 0, 0, r_c_tc, q_ct)
    ...
    def prediction_step(self, x_km1, u, P_km1):
        ...
        dz_sig_km1 = self.generate_sigma_points(np.zeros((self.num_states,1)), P_km1, self.Q)
        ...
        for i in np.arange(dz_sig_km1.shape[1]):
            dx_check[0:3, i:i+1] = dz_sig_km1[0:3, i:i+1] + self.dT * dz_sig_km1[3:6, i:i+1]
            ...
            dx_check[9:12,i] = dz_sig_km1[9:12, i] + self.dT * dz_sig_km1[21:24, i]
            dx_check[12:15,i] = dz_sig_km1[12:15, i] + self.dT * dz_sig_km1[24:27, i]

        # Compute statistics of propagated sigma points
        alpha_0 = self.kappa / (self.kappa + L)
        alpha = 0.5 / (self.kappa + L)
        mu_check =  alpha_0 * dx_check[:,0:1] + alpha * np.sum(dx_check[:,1:], axis = 1)
    ...
    def correction_step(self, x_check, mu_check, P_check, roll, pitch, v_gps, r_c_tc, q_ct):
        ...
        R = self.R_tag # Start with just AprilTag

        # Draw sigma points from stacked state and measurement noise
        dz_sig_k = self.generate_sigma_points(mu_check, P_check, R)
        L = dz_sig_k.shape[0]
        ...
        y_check = np.zeros((6, n_z))

        for i in np.arange(dz_sig_k.shape[0]):
            r_t_vt_i = r_check + dz_sig_k[0:3,i:i+1]
            C_tv_i = np.dot(C_check, exponential_map(dz_sig_k[6:9,i]))
            y_check[0:3,i:i+1] = -np.dot(self.C_vc.T, np.dot(C_tv_i.T, r_t_vt_i) + self.r_v_cv) + dz_sig_k[15:18]
            ...
            y_check[3:6,i:i+1] = quaternion_log(delta_q_ct_i)

        # Calculate statistics of measurement sigma points
        alpha_0 = self.kappa / (self.kappa + L)
        alpha = 0.5 / (self.kappa + L)
        mu_y = alpha_0 * y_check[:,0:1] + alpha * np.sum(y_check[:,1:], axis = 1)
        ...
        delta_x_hat = np.dot(K_k, y_obs - mu_y)
    ...
    def generate_sigma_points(self, mu, P, Q):
        "Draw sigma points from stacked state and noise distribution"
        Z = block_diag(P, Q)
        L = Z.shape[0]
        L_z = np.linalg.cholesky(Z)
        sigma_points = np.vstack((mu, np.zeros((Q.shape[0],1)))) + np.hstack((np.zeros(L, 1), math.sqrt(L + self.kappa) * L_z, -math.sqrt(L + self.kappa) * L_z))
        return sigma_points
